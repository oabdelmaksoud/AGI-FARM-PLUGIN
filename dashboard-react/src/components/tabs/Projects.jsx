...
function exportMarkdown(p, agents) {
  ...
  const lines = [
    `# ${p.name}`,
    `**Status**: ${p.status} | **Progress**: ${p._progress_pct ?? 0}% | **Health**: ${healthScore(p)}`,
...
function AgentChip({ agentId, agents, isOwner }) {
  ...
  return (
    <span className="mono" style={{
      ...
      borderRadius: 6, background: isOwner ? 'var(--cyan-dim)' : 'var(--bg3)',
      border: `1px solid ${isOwner ? 'rgba(0,240,255,0.3)' : 'var(--border)'}`,
      color: isOwner ? 'var(--cyan)' : 'var(--text-secondary)',
      fontWeight: 700
    }}>
...
function HealthDot({ health }) {
  return <span title={`Health: ${health}`} style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', background: HEALTH_COLOR[health], flexShrink: 0 }} />;
}
...
function GanttChart({ project: p }) {
  ...
  if (!milestones.length) return <div className="empty-state">No milestones defined</div>;
  ...
  return (
    <div style={{ position: 'relative', paddingTop: 12 }}>
      <div className="mono" style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: 'var(--text-secondary)', marginBottom: 10, fontWeight: 700 }}>
        <span>Start: {start.toLocaleDateString()}</span>
        <span style={{ color: 'var(--amber)' }}>Today</span>
        <span>End: {end.toLocaleDateString()}</span>
      </div>
      <div style={{ position: 'relative', background: 'var(--bg3)', borderRadius: 8, padding: '16px 0', border: '1px solid var(--border)' }}>
        <div style={{ position: 'absolute', left: `${nowPct}%`, top: 0, bottom: 0, width: 1, background: 'var(--amber)', zIndex: 2, opacity: 0.8 }} />
        {milestones.map(ms => {
          ...
          return (
            <div key={ms.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, padding: '0 16px' }}>
              <div className="mono" style={{
                fontSize: 10, color: isComplete ? 'var(--text-secondary)' : 'var(--text)', width: 160, flexShrink: 0,
                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 700,
              }} title={ms.title}>
                {ms.title}
              </div>
              <div style={{ flex: 1, position: 'relative', height: 12, background: 'var(--bg3)', borderRadius: 3 }}>
                <div style={{
              ...
              <div className="mono" style={{ fontSize: 9, color: 'var(--text-secondary)', width: 60, flexShrink: 0, textAlign: 'right' }}>
...
function BurndownChart({ project: p, tasks }) {
  ...
  if (!total) return <div className="empty-state">No tasks linked</div>;
  const start = new Date(p.created_at || Date.now());
  ...
  const tooltipStyle = { background: 'rgba(8,8,16,0.9)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 10 };

  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.03)" vertical={false} />
        <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: 'var(--text-secondary)' }} />
        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: 'var(--text-secondary)' }} orientation="right" />
        <Tooltip
          contentStyle={tooltipStyle}
          itemStyle={{ padding: '2px 0' }}
        />
        <Legend verticalAlign="top" height={36} iconType="circle" wrapperStyle={{ fontSize: 9, letterSpacing: '0.05em' }} />
        <Line type="monotone" dataKey="ideal" stroke="rgba(255,255,255,0.1)" strokeDasharray="5 5" dot={false} name="Ideal" strokeWidth={1} />
        <Line type="monotone" dataKey="actual" stroke="var(--cyan)" strokeWidth={3} dot={{ r: 2, fill: 'var(--cyan)' }} name="Actual" />
...
function BudgetChart({ project: p }) {
  ...
  return (
    <div className="card" style={{ background: 'var(--bg3)', padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, alignItems: 'flex-end' }}>
        <div>
          <div className="section-title" style={{ marginBottom: 4 }}>Spend</div>
          <div style={{ fontSize: 18, fontWeight: 900, color }}>
            ${spent.toFixed(2)} <span style={{ fontSize: 11, color: 'var(--text-secondary)', fontWeight: 600 }}>/ ${alloc.toFixed(2)}</span>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 8, color: 'var(--text-secondary)', fontWeight: 800, marginBottom: 2 }}>Utilization</div>
          <div className="mono" style={{ fontSize: 14, fontWeight: 900, color }}>{pct}%</div>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={24}>
        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
          ...
          <Bar dataKey="Spent" stackId="a" fill={color} radius={[4, 0, 0, 4]} />
          <Bar dataKey="Remaining" stackId="a" fill="rgba(255,255,255,0.03)" radius={[0, 4, 4, 0]} />
        </BarChart>
      </ResponsiveContainer>
      {pct > 80 && (
        <div style={{
          marginTop: 12, padding: '4px 8px', background: 'var(--red-dim)',
          border: '1px solid var(--red)', borderRadius: 4, display: 'flex', gap: 6, alignItems: 'center'
        }}>
          <span style={{ fontSize: 12 }}>🚨</span>
          <span style={{ fontSize: 9, fontWeight: 800, color: 'var(--red)' }}>Budget threshold exceeded — reduce spending</span>
        </div>
      )}
    </div>
  );
...
function AgentWorkload({ project: p, tasks, agents }) {
  ...
  if (!data.length) return <div className="empty-state">No team metrics</div>;
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(120, data.length * 44)}>
      <BarChart data={data} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }}>
        ...
        <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: 'var(--text)', fontWeight: 800 }} width={100} />
...
function OKRLinks({ project: p, okrs }) {
  if (!okrs?.objectives?.length) return <div className="empty-state">No OKRs configured</div>;
  ...
  if (!linked.length) return <div className="empty-state">No linked OKRs</div>;
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      {linked.map(obj => (
        <div key={obj.id} className="card" style={{ padding: '12px 16px' }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--amber)', marginBottom: 10 }}>🎯 {obj.objective}</div>
          {(obj.key_results || []).map(kr => {
            ...
            return (
              <div key={kr.id} style={{ marginBottom: 8 }}>
                <div className="mono" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, fontSize: 9, fontWeight: 700 }}>
                  <span style={{ color: 'var(--text-secondary)' }}>{kr.result}</span>
                  <span style={{ color: c }}>{kr.current} / {kr.target} {kr.unit}</span>
                </div>
                <div className="progress-track" style={{ height: 4 }}>
                  <div className="progress-fill" style={{ width: `${pct}%`, background: c, height: 4 }} />
                </div>
              </div>
            );
          })}
...
function MilestoneRow({ ms, agents }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 12, flexShrink: 0, color }}>{icon}</span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          fontSize: 12, fontWeight: 700,
          color: isComplete ? 'var(--text-secondary)' : 'var(--text)',
        }}>{ms.title}</div>
        <div style={{ fontSize: 9, color: 'var(--text-secondary)', marginTop: 4, display: 'flex', gap: 12 }}>
          {agent && <span>Owner: {agent.name}</span>}
          {(ms.task_ids || []).length > 0 && <span>{ms.task_ids.length} tasks</span>}
          {ms.auto_complete && <span style={{ color: 'var(--cyan)' }}>auto-complete</span>}
        </div>
      </div>
      {due && <span className="mono" style={{ fontSize: 9, color: due.color, flexShrink: 0, fontWeight: 800 }}>{due.label}</span>}
...
function RiskRow({ risk: r, agents }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 11, color: 'var(--text)', fontWeight: 700, marginBottom: 4 }}>{r.description}</div>
        <div style={{ fontSize: 9, color: 'var(--text-secondary)', display: 'flex', gap: 12 }}>
          {agent && <span>Detected by: {agent.name}</span>}
...
function ActivityItem({ item, agents }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: '1px solid var(--border)' }} className="task-row-hover">
...
function SessionTrace({ sessions, agents }) {
  if (!sessions?.length) return <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>No sessions recorded yet.</div>;
  return (
    <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
      <thead><tr style={{ borderBottom: '1px solid var(--border)' }}>{['Task', 'Agent', 'Proc ID', 'Status', 'Completed'].map(h => <th key={h} style={{ textAlign: 'left', padding: '4px 8px', fontSize: 10, color: 'var(--text-secondary)', fontWeight: 600 }}>{h}</th>)}</tr></thead>
      <tbody>
        {sessions.map((s, i) => {
          ...
          return <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}><td className="mono" style={{ padding: '5px 8px', color: 'var(--cyan)', fontSize: 10 }}>{s.task_id}</td><td style={{ padding: '5px 8px' }}>{agent ? `${agent.emoji} ${agent.name}` : s.assigned_to}</td><td className="mono" style={{ padding: '5px 8px', color: 'var(--text-secondary)', fontSize: 10 }}>{s.proc_id || '—'}</td><td style={{ padding: '5px 8px' }}><span className={`badge ${cls}`}>{s.status}</span></td><td style={{ padding: '5px 8px', color: 'var(--text-secondary)' }}>{s.completed_at ? relTime(s.completed_at) : '—'}</td></tr>;
        })}
      </tbody>
    </table>
  );
...
function ProjectTaskBoard({ tasks, projectTaskIds, agents }) {
  ...
  if (!projTasks.length) return <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>No tasks linked yet.</div>;
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      {cols.map(col => {
              ...
              return (
                <div key={t.id} onClick={() => setExpanded(isExp ? null : t.id)} style={{ marginBottom: 6, padding: '8px 10px', background: 'var(--surface)', border: `1px solid ${isExp ? 'rgba(0,229,255,.3)' : 'var(--border)'}`, borderRadius: 6, cursor: 'pointer' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span className="mono" style={{ fontSize: 10, color: 'var(--text-secondary)', minWidth: 50 }}>{t.id}</span>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{isExp ? '▲' : '▼'}</span>
                  </div>
                  {isExp && (
                    <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
                      {t.hitl_reason && <div style={{ padding: '6px 10px', background: 'var(--purple-dim)', border: '1px solid rgba(224,64,251,.25)', borderRadius: 5, fontSize: 11, color: 'var(--purple)' }}>🚨 {t.hitl_reason}</div>}
                      {t.description && <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{t.description}</div>}
                      {t.output && <div style={{ fontSize: 11, color: 'var(--text)', lineHeight: 1.5, padding: '6px 10px', background: 'var(--green-dim)', border: '1px solid rgba(0,230,118,.15)', borderRadius: 5 }}>✅ {t.output}</div>}
                      <div style={{ display: 'flex', gap: 16, fontSize: 10, color: 'var(--text-secondary)', flexWrap: 'wrap' }}>
...
function ProjectCard({ project: p, agents, selected, onClick }) {
  ...
  return (
    <div onClick={onClick} className="card" style={{
      ...
      borderColor, background: selected ? 'var(--cyan-dim)' : 'var(--bg3)',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontWeight: 900, fontSize: 14, color: selected ? 'var(--cyan)' : 'var(--text)' }}>{p.name}</span>
            </div>
            <div className="mono" style={{ fontSize: 9, color: 'var(--text-secondary)', fontWeight: 700 }}>{p.id} · {p.status}</div>
          </div>
        </div>

        <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 16, height: 36, overflow: 'hidden' }}>{p.description}</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 16 }}>
          {[
            ['Tasks', `${tc.done || 0}/${tc.total || 0}`, 'var(--cyan)'],
            ['Milestones', `${mc.done || 0}/${mc.total || 0}`, 'var(--green)'],
            ['Quality', p._quality_score ? `${p._quality_score}` : '--', 'var(--amber)'],
            ['Velocity', p._velocity ?? '--', 'var(--text-secondary)']
          ].map(([l, v, c]) => (
            <div key={l} style={{ textAlign: 'center', padding: '8px 4px', background: 'var(--bg3)', borderRadius: 6, border: '1px solid var(--border)' }}>
              <div style={{ fontSize: 8, color: 'var(--text-secondary)', fontWeight: 800, marginBottom: 4 }}>{l}</div>
              <div className="mono" style={{ fontSize: 11, fontWeight: 800, color: c }}>{v}</div>
            </div>
          ))}
        </div>
      ...
      {risks.length > 0 && (
        <div style={{
          padding: '8px 16px', background: 'var(--red-dim)', borderTop: '1px solid rgba(255,23,68,0.2)',
          fontSize: 9, color: 'var(--red)', fontWeight: 800
        }}>
...
function ProjectDetail({ project: p, agents, tasks, okrs, onClose, onReplan, onExecute, onSaveBudget, onSaveOkr, lastUpdated }) {
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden', border: `1px solid ${color}44`, marginBottom: 24 }}>
      {/* Header */}
      <div style={{ padding: '24px 30px', background: 'var(--bg3)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 24 }}>
        <ProgressRing pct={pct} size={100} color={color} />
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
            <HealthDot health={health} />
            <span style={{ fontSize: 24, fontWeight: 900, color: 'var(--text)' }}>{p.name}</span>
            <Badge label={p.status} color={STATUS_COLOR[p.status] || 'var(--muted)'} />
            {p.priority_weight && <Badge label={`P${p.priority_weight}`} color="var(--amber)" />}
            <Badge label={`${health} health`} color={HEALTH_COLOR[health]} />
          </div>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 12, maxWidth: 800 }}>{p.description}</div>
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
            <AgentChip agentId={p.owner} agents={agents} isOwner />
            {p.target_completion && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, color: 'var(--text-secondary)' }}>
                <span style={{ color: 'var(--cyan)' }}>Deadline:</span>
                <DeadlineBadge iso={p.target_completion} />
              </div>
            )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button className="btn-primary" onClick={() => onExecute?.(p.id)} style={{ padding: '8px 16px', fontSize: 10 }}>Execute</button>
          <button className="btn-secondary" onClick={() => onReplan?.(p.id)} style={{ padding: '8px 16px', fontSize: 10 }}>Replan</button>
          <button className="btn-secondary" onClick={() => exportMarkdown(p, agents)} style={{ padding: '8px 16px', fontSize: 10 }}>Export</button>
          <button className="btn-ghost" onClick={onClose} style={{ padding: '8px 16px', fontSize: 10 }}>Close</button>
          <LastUpdated ts={lastUpdated} />
        </div>
      </div>

      {/* Grid Summary */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8,1fr)', borderBottom: '1px solid var(--border)', background: 'var(--bg3)' }}>
        {[
          ['Tasks', `${tc.done || 0}/${tc.total || 0}`, 'var(--cyan)'],
          ['Active', tc.active || 0, 'var(--amber)'],
          ['Blocked', tc.blocked || 0, tc.blocked ? 'var(--red)' : 'var(--muted)'],
          ['HITL', tc.hitl || 0, tc.hitl ? 'var(--purple)' : 'var(--muted)'],
          ['Milestones', `${mc.done || 0}/${mc.total || 0}`, 'var(--green)'],
          ['Quality', p._quality_score ? `${p._quality_score}` : '--', 'var(--amber)'],
          ['Velocity', p._velocity ?? '--', 'var(--text-secondary)'],
          ['Spent', `$${p.budget?.spent_usd ?? 0}`, 'var(--cyan)']
        ].map(([l, v, c]) => (
          <div key={l} style={{ textAlign: 'center', padding: '16px 8px', borderRight: '1px solid var(--border)' }}>
            <div style={{ fontSize: 8, color: 'var(--text-secondary)', marginBottom: 6, fontWeight: 800 }}>{l}</div>
            <div className="mono" style={{ fontSize: 15, fontWeight: 900, color: c }}>{v}</div>
          </div>
        ))}
      </div>

      {/* Navigation */}
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg3)', overflowX: 'auto', padding: '0 20px', gap: 4 }}>
        {SECTIONS.map(s => {
      ...
      <div style={{ padding: 30, minHeight: 400 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 30 }}>
            <div>
              ...
              <div className="progress-track" style={{ height: 6, marginBottom: 16 }}>
                <div className="progress-fill" style={{ width: `${mc.total ? Math.round((mc.done / mc.total) * 100) : 0}%`, background: 'var(--green)' }} />
              </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                {(!p._activity || p._activity.length === 0) && <div className="empty-state">No activity yet</div>}
              </div>
            </div>
            {risks.length > 0 && <div style={{ gridColumn: '1/-1' }}><div className="section-title" style={{ color: 'var(--red)' }}>Risks</div>{risks.slice(0, 3).map(r => <RiskRow key={r.id} risk={r} agents={agents} />)}</div>}
          </div>
        )}
        ...
        {section === 'Risks' && <div style={{ maxWidth: 800, margin: '0 auto' }}>{risks.length === 0 ? <div className="empty-state" style={{ color: 'var(--green)' }}>No active risks</div> : risks.map(r => <RiskRow key={r.id} risk={r} agents={agents} />)}</div>}
        {section === 'Team' && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 16 }}>
          {(p.team || []).map(aid => {
            ...
            return (
              <div key={aid} className="card" style={{ borderColor: isOwner ? 'var(--cyan)' : 'var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <span style={{ fontSize: 28 }}>{agent?.emoji || '🤖'}</span>
                  <div>
                    <div style={{ fontWeight: 900, fontSize: 13 }}>{agent?.name || aid}</div>
                    <div style={{ fontSize: 9, color: 'var(--text-secondary)', fontWeight: 700 }}>{agent?.role || 'agent'}</div>
                  </div>
                  {isOwner && <Badge label="Owner" color="var(--amber)" size={7} />}
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  <div style={{ flex: 1, textAlign: 'center', background: 'var(--bg3)', padding: '10px', borderRadius: 8, border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 8, color: 'var(--text-secondary)', marginBottom: 4 }}>Tasks</div>
                    <div className="mono" style={{ fontWeight: 900, color: 'var(--cyan)', fontSize: 13 }}>{agent?.tasks_completed || 0}</div>
                  </div>
                  <div style={{ flex: 1, textAlign: 'center', background: 'var(--bg3)', padding: '10px', borderRadius: 8, border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 8, color: 'var(--text-secondary)', marginBottom: 4 }}>Quality</div>
                    <div className="mono" style={{ fontWeight: 900, color: 'var(--amber)', fontSize: 13 }}>{(agent?.avg_quality || 0).toFixed(1)}</div>
                  </div>
                </div>
              </div>
            );
        ...
        {section === 'Budget' && (
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18 }}>
            ...
            <div className="card" style={{ background: 'var(--bg3)' }}>
              <div className="section-title" style={{ marginBottom: 12 }}>Budget Settings</div>
              <div style={{ display: 'grid', gap: 8 }}>
                ...
                <button className="btn-primary" onClick={() => onSaveBudget?.(p.id, budgetDraft)}>Save Budget</button>
              </div>
            </div>
          </div>
        )}
        {section === 'OKRs' && (
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18 }}>
            ...
            <div className="card" style={{ background: 'var(--bg3)' }}>
              <div className="section-title" style={{ marginBottom: 12 }}>Link OKRs</div>
              <div style={{ display: 'grid', gap: 8 }}>
            ...
            return (
              <div key={d.id} className="card" style={{ padding: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  ...
                  <span style={{ flex: 1, fontWeight: 900, fontSize: 14 }}>{d.decision}</span>
                  <span className="mono" style={{ fontSize: 9, color: 'var(--text-secondary)', fontWeight: 700 }}>{relTime(d.decided_at)}</span>
                </div>
                {d.rationale && <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6, padding: '12px 16px', background: 'var(--bg3)', borderRadius: 8, borderLeft: '2px solid var(--purple)', marginBottom: 12 }}>{d.rationale}</div>}
                <div style={{ display: 'flex', gap: 20 }}>
                  <AgentChip agentId={d.decided_by} agents={agents} />
                  {d.task_id && <span className="mono" style={{ fontSize: 9, fontWeight: 800, color: 'var(--text-secondary)' }}>Task: <span style={{ color: 'var(--cyan)' }}>{d.task_id}</span></span>}
                </div>
              </div>
            );
          })}
    ...
    <div className="card" style={{
      ...
      <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
        ...
        <input
          ...
          style={{ width: '100%', padding: '10px 14px 10px 40px', fontSize: 11 }}
        />
      </div>
      ...
      <div style={{ fontSize: 9, color: 'var(--text-secondary)', fontWeight: 800 }}>Sort:</div>
      <select
        className="btn-secondary"
        value={sortBy}
...
export default function Projects({ data, lastUpdated, toast }) {
  ...
  if (isLoading) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Loading projects...</div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '20px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900 }}>Projects</h1>
          <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginTop: 4 }}>{enriched.length} project{enriched.length !== 1 ? 's' : ''}</div>
        </div>
        <div style={{ display: 'flex', gap: 16 }}>
          <div className="stat-v" style={{ textAlign: 'right' }}>
            <div className="stat-l">Avg Health</div>
      ...
      <div className="card" style={{ marginBottom: 24, padding: 18, borderColor: 'rgba(0,240,255,0.2)', background: 'var(--cyan-dim)' }}>
        <div style={{ fontSize: 10, color: 'var(--cyan)', fontWeight: 800, marginBottom: 10 }}>
          Task Intake
        </div>
        ...
        {lastDecision && (
          <div className="mono" style={{ marginTop: 10, fontSize: 10, color: 'var(--text-secondary)' }}>
