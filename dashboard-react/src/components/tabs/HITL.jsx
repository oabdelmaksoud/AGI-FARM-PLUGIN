...
function HITLCard({ task, agents, onAction }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
            ...
            <span className="mono" style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{task.id}</span>
      ...
      {/* HITL Reason */}
      <div style={{ padding: '12px 14px', background: 'var(--purple-dim)', border: '1px solid rgba(224,64,251,.25)', borderRadius: 7 }}>
      ...
      {task.description && (
        <div>
          <div style={{ fontSize: 10, color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4 }}>Context</div>
      ...
      {/* Note input */}
      <div>
        <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 6 }}>Optional note (sent to agent)</div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => act('approve')} disabled={!!loading} style={{
          flex: 1, padding: '10px', background: 'var(--green-dim)', border: '1px solid rgba(0,230,118,.4)',
        ...
        <button onClick={() => act('reject')} disabled={!!loading} style={{
          flex: 1, padding: '10px', background: 'var(--red-dim)', border: '1px solid rgba(255,23,68,.35)',
...
export default function HITLTab({ data, lastUpdated }) {
  ...
  return (
    <div className="fade-in" style={{ display: 'grid', gap: 14 }}>
      ...
      {resolved.size > 0 && (
        <div style={{ padding: '10px 14px', background: 'var(--green-dim)', border: '1px solid rgba(0,230,118,.2)', borderRadius: 6, fontSize: 12, color: 'var(--green)' }}>
