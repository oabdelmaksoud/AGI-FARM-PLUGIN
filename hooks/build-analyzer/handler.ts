...
interface BuildError {
  type: 'module-not-found' | 'type-error' | 'syntax-error' | 'dependency' | 'unknown';
  file?: string;
  line?: number;
  message: string;
  suggestion: string;
 ...
 */
function buildSucceeded(output: string): boolean {
...
export default async function buildAnalyzerHook(event: ToolEvent): Promise<string> {
...
`;
    ...
    const errorsList = errors.slice(0, 5).map((error, index) => {
      const errorType = error.type.split('-').map(word =>
